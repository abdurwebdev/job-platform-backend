...
class LeverScraper(BaseApiScraper):
    """
    Generic Lever postings scraper. One class, reused for every company
    that hosts its careers page on Lever — add a new source by adding a
    company_slug to the config list, not by writing new code.
    """

    data_key = None  # Lever returns a raw JSON array, not a wrapped object
